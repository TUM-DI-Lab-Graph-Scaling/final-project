...
Similiar to GemNet, we also evaluated DimeNet++ on S2EF with the 200k dataset and 
...
With DimeNet++, as with GemNet, there is an improved runtime of 50\% by enabling 
...
task, Stage 1 had the lowest memory consumption with 60\% less allocated CUDA memory and 
50\% less reserved memory than the baseline. For the IS2RE task, stage 2 with 
optimizer offloading had the lowest memory consumption with 70\% less allocated CUDA 
memory and 50\% less reserved memory than the baseline. In both tasks, stage 0 with 
mixed-precision archieved the best runtime due to the tremendous communication overhead 
of all stage 1 and 2 runs.
